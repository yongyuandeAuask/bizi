...
// {
    ...
    // // 调整箭头大小，确保可见
    // const float arrow_size = (flags & ImGuiComboFlags_NoArrowButton) ? 0.0f : GetFrameHeight() * 0.8f; // 增加箭头大小
    // const ImVec2 label_size = CalcTextSize(label, NULL, true);
    
    // // 计算预览文本大小
    // const char* preview_text = preview_value ? preview_value : "";
    // ImVec2 preview_text_size = CalcTextSize(preview_text, NULL, true);
    // float preview_padding = style.FramePadding.x * 2.0f;
    // float preview_rect_width = preview_text_size.x + preview_padding;
    
    // const float w = CalcItemWidth(); // 占满一行
    // const float total_width = w;
    // const float arrow_area_width = arrow_size + style.FramePadding.x * 2.0f; // 箭头区域宽度
    // const float preview_area_width = (flags & ImGuiComboFlags_NoArrowButton) ? preview_rect_width : (preview_rect_width + arrow_area_width + style.ItemInnerSpacing.x);
    
    // const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(total_width, label_size.y + style.FramePadding.y * 2.0f));
    // const ImRect total_bb(bb.Min, bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    // ItemSize(total_bb, style.FramePadding.y);
    // if (!ItemAdd(total_bb, id, &bb))
        // return false;
    ...
    // // 渲染：整个按钮被白色矩形包裹（作为边框）
    // const ImU32 white_frame_col = GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)); // 白色
    // RenderNavHighlight(bb, id);
    // // 绘制白色边框，但不填充内部，以确保箭头区域有背景色
    // window->DrawList->AddRect(bb.Min, bb.Max, white_frame_col, style.FrameRounding, ImDrawFlags_None, 1.0f); // 边框粗细 1.0f

    // // 绘制左侧标签
    // if (label_size.x > 0)
    // {
        // const ImVec2 label_pos = ImVec2(bb.Min.x + style.FramePadding.x, bb.Min.y + style.FramePadding.y);
        // RenderText(label_pos, label);
    // }

    // // 计算右侧彩色矩形和箭头位置
    // const float arrow_x = bb.Max.x - arrow_size - style.FramePadding.x;
    // const float preview_rect_x = arrow_x - preview_rect_width - style.ItemInnerSpacing.x;
    
    // // 绘制右侧彩色矩形（预览文本背景）
    // if (!(flags & ImGuiComboFlags_NoPreview))
    // {
        // ImU32 preview_rect_col = IM_COL32(75, 190, 255, 235); // RGBA: 75, 190, 255, 235
        // ImRect preview_rect = ImRect(preview_rect_x, bb.Min.y + style.FramePadding.y, 
                                     // arrow_x - style.ItemInnerSpacing.x, bb.Max.y - style.FramePadding.y);
        // window->DrawList->AddRectFilled(preview_rect.Min, preview_rect.Max, preview_rect_col, style.FrameRounding);
        
        // // 绘制预览文本（居中）
        // if (preview_value != NULL)
        // {
            // const ImVec2 preview_text_pos = ImVec2(
                // preview_rect.Min.x + (preview_rect.GetWidth() - preview_text_size.x) * 0.5f,
                // preview_rect.Min.y + (preview_rect.GetHeight() - preview_text_size.y) * 0.5f
            // );
            // RenderText(preview_text_pos, preview_text);
        // }
    // }
...
// // {
    // // // 箭头背景矩形，仅包裹箭头，避免过大
    // // float arrow_bg_padding = 2.0f; // 内边距，可调整
    // // float arrow_bg_width = arrow_size + arrow_bg_padding * 2; // 宽度：箭头大小 + 内边距
    // // float arrow_bg_height = arrow_size + arrow_bg_padding * 2; // 高度：箭头大小 + 内边距
    // // float arrow_bg_x = arrow_x - arrow_bg_padding; // 基于箭头起始位置调整
    // // float arrow_bg_y = bb.Min.y + (bb.GetHeight() - arrow_bg_height) * 0.5f; // 垂直居中
    // // ImRect arrow_rect = ImRect(arrow_bg_x, arrow_bg_y, arrow_bg_x + arrow_bg_width, arrow_bg_y + arrow_bg_height);
    
    // // // 箭头背景色
    // // ImU32 arrow_bg_col = GetColorU32(hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    // // window->DrawList->AddRectFilled(arrow_rect.Min, arrow_rect.Max, arrow_bg_col, style.FrameRounding);

    // // // 绘制白色箭头
    // // ImU32 arrow_col = GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)); // 白色
    // // const float arrow_center_x = arrow_x + arrow_size * 0.5f;
    // // const float arrow_center_y = bb.Min.y + (bb.GetHeight() * 0.5f);
    // // RenderArrow(window->DrawList, ImVec2(arrow_center_x - arrow_size * 0.3f, arrow_center_y - arrow_size * 0.3f), 
                // // arrow_col, ImGuiDir_Down, 0.6f);
// // }
// // 绘制右侧箭头（白色），带有紧凑的背景矩形，并确保箭头在矩形内居中
// if (!(flags & ImGuiComboFlags_NoArrowButton))
// {
    // // 1. 定义箭头背景矩形的大小和位置（保持不变）
    // float arrow_bg_padding = 2.0f; // 内边距
    // float arrow_bg_width = arrow_size + arrow_bg_padding * 2;
    // float arrow_bg_height = arrow_size + arrow_bg_padding * 2;
    // float arrow_bg_x = arrow_x - arrow_bg_padding;
    // float arrow_bg_y = bb.Min.y + (bb.GetHeight() - arrow_bg_height) * 0.5f; // 在按钮内垂直居中
    // ImRect arrow_rect = ImRect(arrow_bg_x, arrow_bg_y, arrow_bg_x + arrow_bg_width, arrow_bg_y + arrow_bg_height);
    ...
    // // 3. 【关键修改】计算箭头在 arrow_rect 内的居中位置
    // ImU32 arrow_col = GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)); // 白色
    // const float arrow_scale = 0.6f; // 箭头缩放因子
    // const float arrow_rendered_size = arrow_size * arrow_scale; // 箭头渲染后的实际大小
    
    // // 计算 arrow_rect 的中心点
    // const ImVec2 arrow_rect_center = arrow_rect.GetCenter();
    
    // // 从中心点反推，计算箭头绘制的起始位置（RenderArrow 从左上角开始绘制）
    // const ImVec2 arrow_pos = ImVec2(
        // arrow_rect_center.x - arrow_rendered_size * 0.5f, // 水平居中
        // arrow_rect_center.y - arrow_rendered_size * 0.5f  // 垂直居中
    // );
    
    // // 4. 绘制居中后的箭头
    // RenderArrow(window->DrawList, arrow_pos, arrow_col, ImGuiDir_Down, arrow_scale);
// }
    // IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // 箭头大小调整
    const float arrow_size = (flags & ImGuiComboFlags_NoArrowButton) ? 0.0f : GetFrameHeight() * 0.8f;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ...
    // **关键修改：占满整个UI的可用宽度**
    float available_width = ImGui::GetContentRegionAvail().x; // 获取当前内容区域的可用宽度
    float label_space = (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f); // 标签所占宽度（包括间距）
    float w = available_width - label_space; // 组合框主体宽度 = 可用宽度 - 标签宽度
    w = ImMax(w, 1.0f); // 确保宽度至少为1，避免负值或零
    
    const float total_width = w;
    const float arrow_area_width = arrow_size + style.FramePadding.x * 2.0f;
    const float preview_area_width = (flags & ImGuiComboFlags_NoArrowButton) ? preview_rect_width : (preview_rect_width + arrow_area_width + style.ItemInnerSpacing.x);
    ...
    // 渲染：白色边框包裹整个组合框
    const ImU32 white_frame_col = GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f));
    RenderNavHighlight(bb, id);
    window->DrawList->AddRect(bb.Min, bb.Max, white_frame_col, style.FrameRounding, ImDrawFlags_None, 1.0f);
    ...
    // 绘制右侧箭头（白色），背景矩形紧凑，箭头居中
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        // 箭头背景矩形，仅包裹箭头
        float arrow_bg_padding = 2.0f;
        float arrow_bg_width = arrow_size + arrow_bg_padding * 2;
        float arrow_bg_height = arrow_size + arrow_bg_padding * 2;
        float arrow_bg_x = arrow_x - arrow_bg_padding;
        float arrow_bg_y = bb.Min.y + (bb.GetHeight() - arrow_bg_height) * 0.5f;
        ImRect arrow_rect = ImRect(arrow_bg_x, arrow_bg_y, arrow_bg_x + arrow_bg_width, arrow_bg_y + arrow_bg_height);
        ...
        // 计算箭头在背景矩形内的居中位置
        ImU32 arrow_col = GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f));
        const float arrow_scale = 0.6f;
        const float arrow_rendered_size = arrow_size * arrow_scale;
        const ImVec2 arrow_rect_center = arrow_rect.GetCenter();
        const ImVec2 arrow_pos = ImVec2(
            arrow_rect_center.x - arrow_rendered_size * 0.5f,
            arrow_rect_center.y - arrow_rendered_size * 0.5f
        );
        RenderArrow(window->DrawList, arrow_pos, arrow_col, ImGuiDir_Down, arrow_scale);
    }
