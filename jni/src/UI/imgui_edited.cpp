...
namespace edited
{
    ...
    bool Checkbox(const char* label, bool* v)
    {
        ...
        GetWindowDrawList()->AddCircleFilled(check_bb.Max - ImVec2(y_size - it_anim->second.circle_offset, y_size) / 2, 8.f, GetColorU32(it_anim->second.circle), 50);
    ...
    bool BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
    {
        ...
        if (hovered && g.IO.MouseReleased[0] || it_anim->second.opened_combo && g.IO.MouseReleased[0] && !it_anim->second.hovered) it_anim->second.opened_combo = !it_anim->second.opened_combo;
        ...
        it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 55) + 20 : 0.f, g.IO.DeltaTime * 12.f);
    ...
    bool subTab(bool selected, ImGuiID id, const char* icon, const ImVec2& size_arg)
    {
        ImGuiWindow* window = GetCurrentWindow();
        if (window->SkipItems) return false;
    ...
    bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
    {
        ...
        if (grab_bb.Max.x > grab_bb.Min.x) {
            ...
            GetWindowDrawList()->AddRectFilled(slider_bb.Min + ImVec2(0, 1), ImVec2(it_anim->second.slow + frame_bb.Min.x - 3 - it_anim->second.circle_pump, grab_bb.Max.y + 10), GetColorU32(c::accent_color), 30.f);
            PopClipRect();

            GetWindowDrawList()->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.slow, grab_bb.Max.y + 5), 11.5f, GetColorU32(c::accent_color), 30.f);
        }


        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(w - 58, -12), frame_bb.Max + ImVec2(0, 1), GetColorU32(c::widget::background), 4.f);
...
static inline float SliderBehaviorCalcRatioFromValue(float v, float v_min, float v_max, float power, float linear_zero_pos)
{
        ...
        if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
        {
            ...
            if (it_anim->second.active)
            {
                ...
                Begin("picker", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize);
                {
                    ...
                    //HDC hdc = GetDC(NULL);
                    ImVec4 color;
					//;= GetPixel(hdc, cursorPos.x, cursorPos.y);

                    if (search_col)
                    {

                        static long int dwTickStart = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
                        if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count() - dwTickStart > 150)
                        {
                            col[0] = color.x / 255.f;
                            col[1] = color.y / 255.f;
                            col[2] = color.z / 255.f;
                            dwTickStart = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
    ...
    bool ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
    {
        ...
        const ImRect rect(pos, pos + ImVec2(width, 38));

        const ImRect clickable(rect.Min + ImVec2(width - 38, 0), rect.Max);
